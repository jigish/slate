...
| `windowHintsShowIcons` | Boolean | Default: `false`. If true, the application's icon will be shown as a background for the letter instead of the rectangle. This is useful if the `windowHintsIgnoreHiddenWindows` is off so that you can know which application a hint is for.|
